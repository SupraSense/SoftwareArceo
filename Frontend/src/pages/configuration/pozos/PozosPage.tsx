...
import { MapPin, Plus, Trash2, Search, ExternalLink } from 'lucide-react';
import { usePozos } from '../../../hooks/usePozos';
import { Button } from '../../../components/ui/Button';
import { Loader } from '../../../components/ui/Loader';
import { ConfirmDialog } from '../../../components/ui/ConfirmDialog';

export const PozosPage: React.FC = () => {
    const { pozos, loading, loadPozos, deletePozo } = usePozos();
    const [searchTerm, setSearchTerm] = useState('');
    const navigate = useNavigate();

    const [isDeleteDialogOpen, setIsDeleteDialogOpen] = useState(false);
    const [deletingPozo, setDeletingPozo] = useState<{ id: string; nombre: string } | null>(null);
    ...
    const filteredPozos = pozos.filter((pozo) =>
        pozo.nombre.toLowerCase().includes(searchTerm.toLowerCase())
    );

    const handleDeleteClick = (id: string, nombre: string) => {
        setDeletingPozo({ id, nombre });
        setIsDeleteDialogOpen(true);
    };

    const handleConfirmDelete = async () => {
        if (!deletingPozo) return;
        setIsDeleting(true);
        try {
            await deletePozo(deletingPozo.id);
        } finally {
            setIsDeleting(false);
            setIsDeleteDialogOpen(false);
            setDeletingPozo(null);
        }
    };
        ...
        <div className="space-y-6 p-6">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-gray-500 dark:text-gray-400 mt-1">
                        Gestione los pozos petrolíferos disponibles para las órdenes de trabajo
                    </p>
                </div>
                <Button
                    onClick={() => navigate('/app/configuration/pozos/new')}
            ...
            <div className="flex bg-white dark:bg-gray-800 p-4 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
                <div className="relative flex-1">
                    ...
                    <input
                        type="text"
                        placeholder="Buscar por nombre del pozo..."
                        value={searchTerm}
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
                <div className="p-6 border-b border-gray-100 dark:border-gray-700">
                    ...
                    <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                        Total: {filteredPozos.length} pozo(s) activo(s)
                    </p>
                </div>
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="border-b border-gray-100 dark:border-gray-700 bg-white dark:bg-gray-800">
                                ...
                                <th className="py-4 px-6 text-sm font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider text-center">
                                    Ubicación
                                </th>
                            ...
                            {loading ? (
                                ...
                                <tr>
                                    <td colSpan={3} className="py-8 text-center text-gray-500">
                                        {searchTerm
                                            ? 'No se encontraron pozos con ese nombre'
                                    ...
                                    >
                                        ...
                                        <td className="py-4 px-6 text-right">
                                            <Button
                                                size="sm"
                                                variant="danger"
                                                onClick={() => handleDeleteClick(pozo.id, pozo.nombre)}
            ...
            <ConfirmDialog
                ...
                title="Desactivar Pozo"
                description={`¿Estás seguro de que deseas eliminar el pozo "${deletingPozo?.nombre ?? ''}"? Esta acción no se puede deshacer.`}
                isHardDelete={false}
